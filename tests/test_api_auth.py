"""Contract tests for /api/v1/auth/session, /api/v1/auth/logout,
/api/v1/me and /api/v1/me/permissions (EPIC-M1.145)."""
...
def test_establish_session_requires_user_id(client):
    response = client.post("/api/v1/auth/session", json={})
    assert response.status_code == 422
    assert response.json()["error"]["code"] == "MRA_VALIDATION_FAILED"


def test_establish_session_returns_real_token(client):
    response = client.post("/api/v1/auth/session", json={"userId": "user-1"})
...
def test_me_returns_user_context_for_valid_session(client):
    session_data = client.post("/api/v1/auth/session", json={"userId": "user-1"}).json()["data"]
...
def test_refresh_rotates_token_and_invalidates_old_one(client):
    ...
    refresh_response = client.post("/api/v1/auth/session", json={}, headers=_bearer(old_token))
