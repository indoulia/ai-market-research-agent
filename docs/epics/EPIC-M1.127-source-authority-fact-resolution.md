...
**Implementation:**
- `app/source_authority_resolution.py`: new, versioned (`RESOLUTION_VERSION = "SAR-001"`) module. New table `resolved_facts` (migration `0103_source_authority_resolution.py`).
...
**Rebase note:** M1.124 and M1.121 both merged to `main` while this branch was in flight, colliding on migration number `0101` (M1.124) then `0102` (M1.121) -- the same known pattern hit repeatedly this session. Rebased onto latest `main` and renumbered this EPIC's migration to `0103_source_authority_resolution.py`, chained after `0102_market_calendar`.

**Verification (real commands run, not fabricated):**
- `python -m pytest tests/test_source_authority_resolution.py -q` -> `12 passed`
- `python -m pytest -q` (full suite, real local Postgres) -> `1291 passed` (grew from 1267 as `main` advanced through M1.121/M1.124 during the rebase)
- `python -m alembic heads` -> single head `0103_source_authority (head)`, chain resolves cleanly
