...
**Implementation:**
- `app/source_authority_resolution.py`: new, versioned (`RESOLUTION_VERSION = "SAR-001"`) module. New table `resolved_facts` (migration `0101_source_authority_resolution.py`).
...
- `python -m pytest -q` (full suite, real local Postgres) -> `1267 passed`
- `python -m alembic heads` -> single head `0101_source_authority (head)`, chain resolves cleanly
