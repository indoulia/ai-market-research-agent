# EPIC-104 — Stock & Setup-Level Calibration
...
## Dependencies
EPIC-079, EPIC-080, EPIC-084, EPIC-086, EPIC-100.
...
**Implementation:**
- `app/segment_calibration.py`: a new, versioned (`SEGMENT_CALIBRATION_VERSION = "SGC-001"`) module. Reuses EPIC-086's own `SEGMENT_SECTOR`/`SEGMENT_MARKET_CAP`/`SEGMENT_HORIZON` names and `discovery_segmentation.classify_market_cap_bucket`, and EPIC-088's own SMA20-distance/volume-ratio bucket thresholds for the new `SEGMENT_SETUP` combination, and EPIC-011's own calibration verdict vocabulary/`MATERIAL_ERROR_THRESHOLD`/`MIN_SAMPLE_SIZE` — nothing redefined.
...
- **Feed validated calibration into Trust Score:** `calibration_error`/`verdict` are propose-only — no write path to `Prediction`, `PredictionTrustScore`, or `TrustControlDecision`; wiring remains a future revision's job, the same posture EPIC-101/EPIC-102/EPIC-103 already established.
