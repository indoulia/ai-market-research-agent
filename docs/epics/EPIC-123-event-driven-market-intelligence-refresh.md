# EPIC-123 — Event-Driven Market Intelligence Refresh
...
- Trigger targeted re-fetch and re-analysis through EPIC-121.
...
## Dependencies
EPIC-077, EPIC-090, EPIC-094, EPIC-106, EPIC-121.
...
**Design:** composes EPIC-106's `process_event_triggers_for_stock` (`app/event_driven_refresh.py`) for all detection/materiality/mapping/dedup/provenance scope items, and EPIC-121's `app/schedule_orchestration.py` for the one thing EPIC-106 didn't yet have: an actual orchestrated invocation point.
- `run_event_driven_refresh(session, stock_id, *, as_of, trigger_source)` wraps one call to `process_event_triggers_for_stock` in `acquire_execution`/`complete_execution`/`fail_execution` under `OPERATION_EVENT_TRIGGER_PROCESSING` — satisfying "trigger targeted re-fetch and re-analysis through EPIC-121" literally, not just by convention.
- **Concurrency / rate control:** EPIC-121's DB-enforced lock means two overlapping calls for the same stock can't both run at once; the second raises `ConcurrentExecutionError`, propagated to the caller.
- **Two dedup layers, deliberately distinct:** EPIC-106's own `(event_type, source_table, source_id)` constraint dedupes *events*; this module's `dedup_key` (via `trigger_source`, which callers must make unique per invocation) dedupes *invocations* — guards against a retried webhook or a scheduler double-fire re-running the same call, without ever treating "a new event arrived" as a duplicate (verified by `test_a_new_trigger_source_reprocesses_and_picks_up_new_events`).
- **Provider disagreement/fallback:** composed transitively — `process_event_triggers_for_stock` → EPIC-105's `evaluate_prediction_freshness` → EPIC-103's fundamental-provider-disagreement check. Not reimplemented here.
- **Backlog visibility:** `get_pending_event_backlog` surfaces `EventTriggerRecord` rows with `processed_at IS NULL`, which EPIC-106's own two-phase commit (trigger creation committed separately from the evaluation loop) can leave behind after a failed run — exactly the case `fail_execution` records.
...
**Also fixed along the way (separate PR #231, unrelated to this EPIC's own scope but blocking anyone chaining a migration after EPIC-121):** EPIC-121's migration revision id (`0097_event_schedule_orchestration`, 33 chars) exceeded `alembic_version.version_num`'s `VARCHAR(32)`, breaking fresh-database `alembic upgrade head`. Reported by `market-agent-m1-91`; fixed by renaming to `0097_event_sched_orchestration` (30 chars), no schema change.

**Not built (deliberately out of scope):** any new detection source beyond what EPIC-106 already ingests (news, corporate actions, price/volume shock, regime transitions) — "material market/sector movement" and "provider/data correction" event classes beyond what EPIC-106's existing thresholds already cover would need their own ingestion/detection EPIC, not a re-implementation here.
