...
autonomous/epic-m1-24, branched cleanly from `main` (all three declared dependencies -- EPIC-072, EPIC-015, EPIC-016 -- are already merged).
...
- **No write path to `Prediction`, `RecommendationGeneration`, or `ScanCandidate`** -- consensus (EPIC-008), scoring (EPIC-009), and horizon selection (EPIC-010) are called as pure functions directly (`evaluate_positive_consensus`, `compute_positive_opportunity_score`, `select_horizon`), never through `generate_recommendation_for_candidate`, which persists. This is what makes "changing historical production records" (non-goal) structurally impossible rather than merely avoided by convention.
...
- `docs/epics/EPIC-073-historical-recommendation-replay.md` — this completion report.
