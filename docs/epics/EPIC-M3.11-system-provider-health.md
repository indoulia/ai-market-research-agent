...
split-track `M1.142` equivalent for *this* product surface (the old
`M1.142` is "Feedback/Preferences UI", unrelated) — unlike most sibling
...
- Per-`(data_type, provider_id)` reliability, verdicts (`OK`/`WEAK`/
  ...
  (EPIC-M1.93), `compute_provider_quality_report`.
- Outage-severity history (`NONE`/`PARTIAL`/`TOTAL`) naming exactly which
  ...
  (EPIC-M1.114), `record_outage_snapshot`/`get_latest_outage_snapshot`.
- Freshness policy per data type (`FRESHNESS_POLICY`) and the immutable
  ...
  `app/refresh_policy.py` (EPIC-M1.35/M1.93).
- Ingestion-latency degradation history — `app/information_latency.py`
  (EPIC-M1.126), `LatencyDegradationReport`.
- Unexpected market closures — `app/market_calendar.py` (EPIC-M1.121),
  `MarketUnexpectedClosure`.
- NSE weekday/session-window classification with no DB dependency —
  `app/schedule_orchestration.py::classify_session` (EPIC-M1.118); no
...
- `api/schemas/system.py` — `Freshness`, `ProviderStatus`,
  ...
  actually carries. `qualityScore` is M1.93's own `success_rate`;
  `failureRate` is `1 - success_rate`.
- `api/services/system_health.py`:
  - `get_provider_status` — per-provider grid row: M1.93's verdict/
    quality-score/cost report, plus this module's own new
    per-`(data_type, provider_id)` last-successful-fetch and average-
    ingestion-latency lookup (the two fields M1.93's report doesn't
    carry), plus `fallbackActive` from M1.114's latest outage snapshot
    for that data type.
  ...
  - `get_system_health` — overall `OK`/`DEGRADED`/`OUTAGE` rollup: a DB
    connectivity check (same `SELECT 1` M1.132's `/health` already
  ...
  - `get_system_events` — merges M1.114's outage-severity history
    (excluding `NONE`), M1.121's unexpected-closure log and M1.126's
  ...
  - Live provider health pings are honestly out of scope: no
    ...
    `DataFetchAttempt` verdict, the same posture M1.93's own docstring
...
- Flutter: a fourth "System" tab on the existing Settings destination
  ...
  Settings/History, not a seventh primary nav destination (EPIC-M1.134's
  ...
  timeline ("Load more", mirroring EPIC-M3.10's `FeedbackHistoryScreen`
...
- No live provider health ping / no credentialed provider registry in
  the API process — the same explicit, honest gap M1.93 itself
