# EPIC-124 — Market Calendar & Operational Window Management
...
`app/market_calendar.py` supplies the canonical holiday/special-session data EPIC-121's `schedule_orchestration.classify_session` always accepted but never had a source for (EPIC-121's own docstring flagged this as an honest gap deferred to this EPIC):
