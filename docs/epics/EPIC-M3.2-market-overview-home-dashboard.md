...
track's M1.135 (Recommendations Query API) + M1.136 (Recommendation
...
loading, empty/error states. Diffing M3.2's own explicit API contract and
UI Scope line-by-line against that merged code (not against the general
product idea) found this EPIC is **not** ~95% satisfied like M3.1 was —
...
**Already satisfied by existing M1.135/M1.136 (+M1.139/M1.147) work —
...
- Top positive opportunities with price/target/SL/upside/horizon/score/
  ...
  (`MraBreakpoints`-driven 1/2/3-column grid, unchanged from M1.136),
...
- Navigation into the full recommendation detail view on card tap —
  unchanged `context.push('/home/recommendation/${id}')` route (M1.138's
  `RecommendationDetailScreen`).
- Positive-only, immutable-prediction product constraints — inherited
  unchanged from M1.135's `RECOMMENDATION_LABEL = "POSITIVE_OPPORTUNITY"`
  and M1.15's open-lifecycle-only feed; nothing in this EPIC touches
...
- Underlying market-regime/index/news/event/trust-history data sources —
  M1.139's `market.py`/`news_events.py` services and M1.147's
...
- `GET /api/v1/dashboard/snapshot` (query: `market`, `horizon`, `limit`,
  ...
  (M1.135, `sort=score` for `topOpportunities`, `sort=updatedAt` for
  `recentChanges`), `get_market_summary` (M1.139) for
  `marketStatus`/`marketRegime`/`indices`/`asOf`, `list_news`/`list_events`
  (M1.139) merged server-side into one time-ordered `importantEvents`
  feed, and `get_summary` (M1.147, fixed 30d window) for `trustSummary`.
...
- `sector`/`marketCapBucket` query params on the snapshot endpoint: the
  ...
  surface. Added both as additive, optional params reusing M1.135's exact
...
- Flutter: `flutter_app/lib/features/dashboard/dashboard_snapshot.dart`
  ...
  Quick filters (horizon — reusing M1.136's existing `HorizonSelector`;
  market and size-bucket — reusing M1.140's `MraFilterBar` chip pattern;
...
- Scrolling past the snapshot's bounded `topOpportunities`: the snapshot
  ...
  filtered `/recommendations` page 1 to recover a real M1.135 keyset
...
- A genuinely pre-existing, unrelated (M1.147) test-infra fragility:
  ...
  M1.147 tracking test later in the run (reproduced deterministically via
...
- `indices[]` stays always `[]` and `marketStatus` stays always
  `"UNKNOWN"` — both inherited, honestly, from M1.139's own named gaps (no
...
- No new "important events materiality ranking" logic: `importantEvents`
  merges M1.139's `/news`/`/events` by recency only (same as the existing
  M1.140 UI's client-side merge), since no cross-source materiality
...
- No automatic infinite-scroll on the Home dashboard (unlike the old
  M1.136 screen it replaces): a compact "first screen" overview with a
