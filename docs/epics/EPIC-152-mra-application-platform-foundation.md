# EPIC-152 — MRA Application Platform Foundation
...
was renumbered from a collision (was `EPIC-135` in the newer combined
roadmap) with the older, already-implemented split track that also owns a
`EPIC-135` (API Contract & BFF Foundation, `DONE`/merged PR #147),
`EPIC-136` (Flutter Design System, `DONE`) and `EPIC-137` (Flutter App Shell &
...
session's job was to diff EPIC-152's acceptance criteria against that existing,
merged code, not to build a second, parallel Flutter app or API.

**Already satisfied by existing EPIC-135/EPIC-136/EPIC-137 (+ later M1.13x)
...
- Central navigation/routing — `flutter_app/lib/app_shell/app_router.dart` (`go_router` `StatefulShellRoute`, one branch per destination, auth-gated redirect logic from EPIC-149).
...
- Versioned API envelope/error contract, correlation/request ID, pagination/sort conventions, RFC-style validation errors, health endpoint — `api/envelope.py`, `api/request_context.py`, `api/pagination.py`, `api/exception_handlers.py`, `api/routers/health.py` (`GET /api/v1/health`), all from EPIC-135, unchanged.
- Authentication/session context integration point — `api/deps.py` (`get_optional_bearer_subject`/`require_bearer_subject`), enforced for real by EPIC-148's auth router (`api/routers/auth.py`), already merged.

**Genuine gap found and implemented this session:** EPIC-135 shipped
`GET /api/v1/health` and a combined `GET /api/v1/app/bootstrap` (version +
capabilities together), but EPIC-152 explicitly calls out `GET /api/v1/version`
...
- No Flutter-side consumer of the new `/version`/`/capabilities` endpoints was added: the app already gets both pieces of data from the one `GET /api/v1/app/bootstrap` call it makes at cold start (EPIC-149's splash flow); the new endpoints exist for external/tooling callers that want just one concern (e.g. a monitoring probe), per the EPIC's own framing of them as "representative endpoints" for API discovery, not as a mandate to change how the app itself bootstraps.
- ETag/Last-Modified caching remains deferred (unchanged from EPIC-135's own explicit deferral) — still no endpoint with cacheable, slowly-changing data that needs it.

**Conclusion:** EPIC-152's scope was ~95% already satisfied by the
existing, merged EPIC-135/EPIC-136/EPIC-137 (+ EPIC-144/EPIC-148/EPIC-149) work. The
