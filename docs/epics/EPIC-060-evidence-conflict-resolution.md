# EPIC-060 — Evidence Conflict Resolution
...
autonomous/epic-m1-65, branched cleanly from `main` (the declared dependency -- EPIC-059 -- is already merged).
...
Given this platform's real evidence sources (EPIC-043), a literal fact-vs-fact contradiction between two evidence categories cannot happen -- there is no second, independent source for the same fact. `resolve_evidence_conflicts` instead detects the two genuine conflict shapes this platform's data actually supports:
1. **Untrusted-source conflict**: an evidence item that appears `AVAILABLE`/`STALE` (EPIC-043) whose underlying category EPIC-059 has separately assessed as untrusted -- the evidence *looks* present, but the reliability layer says not to trust it (scope: "compare source reliability and freshness").
2. **Revalidation conflict**: EPIC-057's most recent revalidation outcome for this prediction is `WITHDRAWN` or `UPDATED` -- a materially different signal than the "still fully valid" assumption a still-open recommendation currently rests on.

### No Source Silently Discarded

Every EPIC-043 evidence category present for the prediction is recorded in `evidence_categories_considered`, whether or not it contributed a conflict (AC: "no source is silently discarded"), proven directly by test.
...
- `docs/epics/EPIC-060-evidence-conflict-resolution.md` — this completion report.
...
- [x] No source is silently discarded (`evidence_categories_considered` always includes every EPIC-043 category present; proven by test).
