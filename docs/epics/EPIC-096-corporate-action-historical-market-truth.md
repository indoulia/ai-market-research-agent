# EPIC-096 — Corporate Action & Historical Market Truth
...
autonomous/epic-m1-96, branched cleanly from `main` (both declared dependencies -- EPIC-075, EPIC-095 -- are already merged).
...
- `docs/epics/EPIC-096-corporate-action-historical-market-truth.md` — this completion report.
...
- [x] Prediction outcomes remain reproducible after data corrections (a `CorporateAction` is itself immutable once recorded, and `PredictionOutcome` remains immutable per EPIC-005/EPIC-095 -- a correction is always a new, later-recorded action, never a rewrite of an already-computed outcome).
