# EPIC-025 — Candidate Model Evaluation
...
## Dependency Chain
**Previous:** EPIC-073, EPIC-074, EPIC-022, EPIC-023, EPIC-024  
**Next:** EPIC-026
...
autonomous/epic-m1-30, branched cleanly from `main` (all five declared dependencies -- EPIC-073, EPIC-074, EPIC-022, EPIC-023, EPIC-024 -- are already merged).
...
- **No new table or migration.** Pure read-side aggregation, restricted to an `EvaluationWindow` (EPIC-074).
- **This repo still has no second, real candidate model** -- the same documented caveat EPIC-074 and EPIC-024 already carry. This EPIC builds the comprehensive comparison machinery two disjoint historical windows need (return, calibration, and every segmentation dimension the scope names), genuinely usable today for comparing two eras of the same code/model; once a real second model exists, "baseline window" and "candidate window" become "baseline model's period" and "candidate model's period" with zero changes to this module.
- **`compute_window_evaluation(session, window) -> WindowEvaluation`** re-derives the windowed aggregate directly rather than retrofitting a `window` parameter onto EPIC-006/EPIC-071/EPIC-072/EPIC-022/EPIC-023's existing (global, all-time) report functions -- extending each would have meant modifying several already-merged modules. This is the same scoping choice EPIC-074 and EPIC-024 already made for their own windowed queries.
...
- **Horizon segmentation always reports all four `VALID_HORIZON_DAYS`**, even with zero samples, matching this platform's established "no dimension silently omitted" convention. **Sector/market-cap (via EPIC-029's `DiscoverySegment`), discovery source (via EPIC-020's `DiscoveryRecord`), and regime (via EPIC-021's `MarketRegime`) are reported "where available"** -- none of those three are universally populated for every historical `Prediction`, the same honest-partial-coverage pattern established in EPIC-072/EPIC-074/EPIC-022/EPIC-023/EPIC-024.
...
- **`OverlappingEvaluationWindowsError`** (reused from EPIC-074) is raised if the two windows overlap -- "evaluation data is strictly out-of-sample" (AC) is a hard error, not a warning.
- **`REGRESSION_MARGIN = Decimal("0.10")`** (fixed, documented, versioned via `CANDIDATE_MODEL_EVALUATION_VERSION`): the same regression-detection concept and value EPIC-074 already established for its own simpler comparison, reused here for consistency across the platform's comparison EPICs.
...
- `docs/epics/EPIC-025-candidate-model-evaluation.md` — this completion report.
