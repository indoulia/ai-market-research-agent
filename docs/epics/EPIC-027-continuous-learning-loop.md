# EPIC-027 — Continuous Learning Loop
...
- Invoke the EPIC-026 promotion gate.
...
- Promoting a model without the EPIC-026 evidence gate.
...
## Dependency Chain
**Previous:** EPIC-023, EPIC-024, EPIC-025, EPIC-026  
...
autonomous/epic-m1-32, branched cleanly from `main` (all four declared dependencies -- EPIC-023, EPIC-024, EPIC-025, EPIC-026 -- are already merged). This EPIC completes the `EPIC-021 → ... → EPIC-027` learning chain.
...
- **"Every promoted model has traceable evaluation evidence" (AC)** holds through the chain `LearningCycle.model_promotion_id → ModelPromotion.evidence_report_version → EPIC-025's comparison report` -- and `LearningCycle` itself additionally records `discovery_effectiveness_version`/`calibration_candidate_version` so a full cycle's refreshed evidence is traceable even beyond the promotion decision alone. **"Failed candidates remain available for analysis"** holds because `ModelPromotion` (EPIC-026) never deletes a `REJECTED` row, and this module never touches that table's rows once written.
...
- `docs/epics/EPIC-027-continuous-learning-loop.md` — this completion report.
...
- [x] Every promoted model has traceable evaluation evidence (`model_promotion_id` → EPIC-026 → EPIC-025's comparison report).
- [x] Failed candidates remain available for analysis (EPIC-026's `ModelPromotion` rows are never deleted or touched by this module).
