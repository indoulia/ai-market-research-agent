# EPIC-110 — Prediction Lifecycle & Recommendation Capacity Control
...
## Dependencies
EPIC-050, EPIC-081, EPIC-118, EPIC-099, EPIC-105.
...
- **Define CREATED/ACTIVE/REVISED/EXPIRED/TARGET_HIT/SL_HIT/INVALIDATED/EVALUATED states:** `classify_prediction_lifecycle_state` is a pure, read-only classifier over already-immutable evidence — EPIC-005's `PredictionOutcome`, EPIC-050's `RecommendationRevision` history, EPIC-057's `RecommendationRevalidationOutcome`, and EPIC-018's own `RecommendationLifecycle` row — in a fixed priority order (closed outcome > revalidation verdict > revision > open lifecycle tracking > `CREATED`). This is the same "derived-classification-never-delete" pattern this platform already uses for retention/archiving/delisting.
...
- **Define configurable recommendation capacity limits / rank before publication:** reads EPIC-118/EPIC-099's own already-persisted `PositiveOpportunityRanking` (`included=True`, ordered by `rank_position`) for the scan — never re-ranks — and cuts off at a caller-supplied `capacity_limit` (default `DEFAULT_CAPACITY_LIMIT = 10`).
