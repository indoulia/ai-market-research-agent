# EPIC-144 — Feedback, Preferences & Learning Boundary API
...
## Parallelization
API implementation. UI EPIC-145 consumes this exact contract.

## Dependencies
EPIC-135, EPIC-119, EPIC-133, EPIC-110.

**Dependency note (2026-08-21):** EPIC-135/EPIC-119 are `DONE`. EPIC-133
(prediction abstention quality) and EPIC-110 (prediction lifecycle/capacity
...
already-existing EPIC-041/EPIC-047/EPIC-055 domain modules, not consumers of
abstention or capacity signals.

## Completion Report (2026-08-21)

**Implemented:**
- `GET`/`PUT /api/v1/preferences` — composes EPIC-041's `app.user_preferences`
  (horizon/risk/min-confidence/sectors/market-cap-buckets, reused
  unchanged), EPIC-055's `app.recommendation_alerts` (`muted_alert_types` for
  ...
  simplification of EPIC-041's richer `horizon_band` concept: every `PUT`
  sets `horizon_band=CUSTOM, custom_horizon_days=defaultHorizon`, so the
  value round-trips exactly and is still validated against EPIC-009's real
  ...
  subject` — the EPIC-135-established, not-yet-verified bearer-token
  extraction point; real verification is EPIC-148) since they are
  inherently per-user state, isolated per user (verified by test).
- `POST /api/v1/recommendations/{id}/feedback` — composes EPIC-047's
  ...
  without a key correctly produces two, preserving EPIC-047's own "multiple
  ...
  version (via EPIC-050's `get_active_version`, matching EPIC-140's
...
no-key repeats producing two (per EPIC-047's own guarantee). Plus 1 updated
...
(the bearer token is a self-asserted, unverified identity until EPIC-148);
any actual learning-pipeline consumption of `learningImpact="queued"`
feedback (EPIC-133/EPIC-110 don't exist yet, and wiring feedback into
