...
- **EPIC-008 — Positive Consensus Engine**
- **EPIC-009 — Positive Opportunity Scoring**
- **EPIC-010 — Positive Horizon Selection**
...
autonomous/epic-m1-19, branched cleanly from `main` (all five declared dependencies -- EPIC-067, EPIC-008, EPIC-009, EPIC-010, EPIC-016 -- are already merged).

### Objective

Evaluate active watchlist (EPIC-067) stocks through the identical positive-consensus/scoring/horizon pipeline internally scanned and externally discovered candidates already use, so watchlist membership alone can never create a recommendation.

### Design Decisions

- **No new table or migration.** This EPIC's real content -- "reuse the existing consensus/score/horizon pipeline" -- is already fully embodied by EPIC-016's `generate_recommendation_for_candidate`, reachable via EPIC-020's `record_discovery` + `route_discovery_through_pipeline`. `app/watchlist_analysis.py`'s `analyze_watchlist_stock` is a thin wrapper: verify the stock is currently active on the watchlist (EPIC-067's `is_active`), then call the exact same two EPIC-020 functions EPIC-020 itself and EPIC-028 already call, tagged `SOURCE_WATCHLIST` (added to `app/discovery.py` alongside `SOURCE_CHATGPT`/`SOURCE_DAILY_UNIVERSE_SCAN`).
- **Deliberately not built on `app/watchlist.py` (EPIC-007)'s `evaluate_watchlist_candidate`.** That function only reuses EPIC-008's consensus contract; score and horizon are caller-supplied kwargs, not computed. This EPIC's scope explicitly requires reusing EPIC-009's opportunity score and EPIC-010's horizon selection too (scope items 3-4), which only EPIC-016's generator provides end to end. `app/watchlist.py` is untouched by this EPIC.
- **"Point-in-time market/model data" (scope item 1) comes from the `ScanCandidate` row** already computed for that stock in a given `scan_id` (EPIC-015) -- a watchlist stock can only be added if its underlying `Stock` is part of the active universe (EPIC-067's own validation), so every active watchlist stock is, by construction, a candidate in each day's universe scan once that scan has run.
- **`StockNotOnWatchlistError`** is this EPIC's one genuinely new piece of behavior: `analyze_watchlist_stock` refuses to run for a stock that is not *currently* active on the watchlist (per EPIC-067's derived state, not a point-in-time snapshot), before touching the pipeline at all -- proven directly by tests for both "never watchlisted" and "watchlisted then removed."
- **Failure-mode reuse, not special-casing:** a scan-excluded stock (`stale_market_data`, etc.) raises EPIC-016's own `CandidateNotEligibleError` unchanged; there is no watchlist-specific handling of that case, matching how EPIC-020 already treats the identical situation for externally discovered candidates.
...
- `app/discovery.py` — added `SOURCE_WATCHLIST` constant (no behavior change to existing EPIC-020 functions).
- `tests/test_watchlist_analysis.py` — new: 7 tests.
- `docs/epics/EPIC-068-watchlist-positive-analysis.md` — this completion report.
...
- `git diff --check`
- `alembic heads` (no migration added by this EPIC; head unchanged from EPIC-067's `0018_watchlist_entries`)
...
- [x] Watchlist candidates use the same quantitative path as normal candidates (identical `generate_recommendation_for_candidate` call, via EPIC-020's routing).
...
I believe this implementation satisfies all six acceptance criteria with real, verified evidence. The decision to build on EPIC-020's routing machinery rather than EPIC-007's watchlist evaluator is a documented design choice driven directly by the scope's explicit requirement to reuse EPIC-009/EPIC-010, which only EPIC-016's generator provides. Per the user's standing-contract update, Claude will merge this PR once CI is green and it is cleanly mergeable, then continue to the next eligible EPIC.
