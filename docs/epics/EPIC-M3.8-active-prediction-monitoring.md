...
already includes EPIC-M3.5 via PR #268). `autonomous/epic-m3-6` and
...
- `app/prediction_outcome_monitor.py` (EPIC-M1.119, merged PR #235) is
  ...
  `get_event_history`. M1.119's own completion report explicitly named
...
- `api/routers/recommendations.py`/`api/services/recommendations.py`
  (EPIC-M1.135) already define the "live feed" eligibility this EPIC
  reuses verbatim: latest included `PositiveOpportunityRanking` batch +
  `RecommendationLifecycle.state in OPEN_STATES` (M1.15).
- `app/market_calendar.py` (EPIC-M1.121, `count_trading_days`,
  `get_holiday_dates_in_range`) and `app/schedule_orchestration.py`
  (EPIC-M1.118, `classify_session`, `OPERATION_PRICE_MONITORING`'s 15-
  minute cadence) — both landed after M1.137 was written (that EPIC's own
  docstring says its `expiryAt` is "a naive calendar-day estimate ...
  since M1.121 hasn't landed") and are reused here for a real,
  trading-day-aware `remainingTradingDays` and an honest
  `nextEvaluationAt` estimate.
- `app/recommendation_revision.get_revision_history` (EPIC-M1.55) for
  `lastRevisionAt`.
- Flutter design-system components reused as-is: `MraCard`, `MraChip`,
  `MraStateView`, `SkeletonCard`, `showMraBottomSheet`, `MraTypography`,
  `MraColorScheme` (all EPIC-M1.133).
- No pre-existing UI screen already covered this: EPIC-M1.136's dashboard
  cards show target/SL as static prices only (no live distance/progress/
  horizon-remaining/status), and EPIC-M1.147/148's "Tracking" screen is a
...
- `api/services/predictions_active.py` — `list_active_predictions`/
  `get_active_prediction`: joins the same M1.135 eligibility query,
  enriches each row with M1.119's `status` (read-only — this API layer
  ...
  staleness, revision-identity edge case inherited from M1.135's own list
  query).
...
- Flutter: `flutter_app/lib/features/tracking/active_prediction.dart`
  ...
  text, status chip covering every M1.119 state). `tracking_repository.dart`
...
# M1.119 TARGET_HIT/STOP_LOSS_HIT status (not recomputed), price-relative
...
# All tests passed! (155 tests, incl. 5 new "EPIC-M3.8 active positions
# section" cases in tracking_screen_test.dart: empty state, full card
# rendering, M1.119-sourced status label, fresh-refetch-on-tap detail
...
- `nextEvaluationAt` is a best-effort estimate (next
  ...
  call `evaluate_prediction_realtime` in production yet (M1.119's own
...
- `lastRevisionAt`/ranking-score resolution inherits the same
  no-`get_active_version`-resolution characteristic EPIC-M1.135's own
...
**Conclusion:** EPIC-M3.8's UI scope and both named API endpoints are
implemented, tested and wired end-to-end, composing entirely from
already-merged domain modules (M1.15/M1.55/M1.87/M1.118/M1.119/M1.121/
M1.133/M1.135) per the EPIC's own AC. Marking this EPIC `DONE`.
