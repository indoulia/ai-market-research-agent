# EPIC-134 — End-to-End Prediction Validation Evidence Gate
...
## Dependencies
EPIC-117, EPIC-125, EPIC-126, EPIC-127, EPIC-128, EPIC-129, EPIC-130, EPIC-131, EPIC-132, EPIC-133.
...
- `app/end_to_end_validation_gate_v2.py` (E2E-131-001): `compile_end_to_end_validation_report`
  ...
  capability, mirroring EPIC-117's own posture exactly (a check with no data is
  `INSUFFICIENT_EVIDENCE`, never a silent pass; overall `READY_FOR_PRODUCTION_V2` requires every
  check to be an explicit `PASS`).
  - `RELEASE_READINESS_V1` embeds EPIC-117's own `compile_release_readiness_report` verdict directly
  ...
  - `HORIZON_COVERAGE` validates this platform's actually-supported horizons
    ...
    string.
  - `PURGED_EMBARGOED_VALIDATION` reads EPIC-128's `TemporalValidationPolicyDecision.verdict`.
  - `EXECUTION_COST_ASSUMPTIONS` requires both EPIC-098's `ExecutionCostAssessment` and EPIC-131's
  ...
  - `TARGET_STOP_HORIZON_CLOSURE` reads this session's own EPIC-122 `PredictionOutcomeEvent` terminal
    states.
  - `EVENT_DRIVEN_REVISION_AND_FRESHNESS` requires both EPIC-105's `PredictionFreshnessDecision` and
    this session's own EPIC-129 `InformationLatencyAssessment`.
  - `PROVIDER_PROVENANCE` reads EPIC-130's `ResolvedFact`.
  - `CHAMPION_CHALLENGER_SHADOWING` reads this session's own EPIC-126
    `ShadowChallengerComparisonReport`/`ChampionRollback`.
  - `TRUST_SCORE_RISE_AND_REGRESSION` requires EPIC-062's regression-check history to contain *both*
    a `HEALTHY` and a `REGRESSED` verdict -- demonstrating both rise and regression cases, not just
    one direction (AC).
  - `POSITIVE_ONLY_AND_ABSTENTION` reads EPIC-133's `SegmentAbstentionQualityReport`.
  - `IMMUTABLE_HISTORY_AND_REPLAY` requires both EPIC-081's `DailyPredictionSnapshot` and EPIC-115's
    `ReplayRun`.
  - `PORTFOLIO_AND_CROSS_SECTIONAL_RANKING` requires both EPIC-099's `RankingEffectivenessReport` and
    this session's own EPIC-127 `PortfolioSelectionEffectivenessReport`.
  - `MODEL_PROVIDER_COST_VS_VALUE` reads EPIC-093's `CostQualityTradeoffReport`.
  - Always computes and persists a fresh, independent report row -- never idempotent, matching the
    "report" posture other multi-source aggregation EPICs (EPIC-099/EPIC-117/EPIC-127) already use.
...
- "Regime/horizon/sector/size breakdowns" (Required Evidence) are partially covered per-check
  (e.g. `by_horizon` inside EPIC-126's own comparison reports) but this gate does not itself
