...
navigation) was already ~85% built by the older, merged M1.139 (API) and
M1.140 (UI) EPICs. This session's job was to diff M3.5's specific scope
against that existing code, not build a second feed or a second API.

**Already satisfied by existing M1.139/M1.140 work — verified, not
reimplemented:**
...
- Named, honest dedup gap carried forward unchanged from M1.139: same-source
  ...
  scope (would need fuzzy content matching — M1.127's territory, still not
...
2. **`eventType`/`materiality`/date-range query filters** — M1.139 only
   ...
   - **Deliberately not added:** a `materiality` query param on `/events`.
     ...
     have always reported `materiality: null` (unchanged since M1.139).
     ...
     same "honest gap, not fabricated" standard M1.139 itself used.
3. **UI filters** (event type, materiality, date) — `NewsEventsScreen` had
   ...
   M1.133 component, already used by `discover_screen.dart` — no new
   ...
   toggle EPIC-M1.143 already added to `NewsEventsRepository`); Materiality
...
4. **"Corporate-action and earnings sections"** — implemented as the Type
   ...
   permanently-visible split sections, preserving M1.140's deliberate
...
5. **Event cards: icon + affected-symbol chips** — `NewsCard` (shared
   M1.133 component) gained optional `icon`/`affectedSymbols` params
   ...
   .affectedSecurities` data, always a single symbol today per M1.139's own
...
- No `predictionRevisionIds[]`/`sources[]`/`impactStatus`/`eventId`-named
  response schema as this doc's own "Event response" sketch lists. M1.139's
...
- No cross-source syndicated-story dedup (unchanged M1.139/M1.127 gap, see
  above).
...
- `page`/`pageSize` offset pagination this doc's Query line mentions was
  ...
  M1.139 already uses for these two ever-growing tables — cursor pagination
...
- `flutter_app/test/features/news_events/news_events_screen_test.dart` — 2
  ...
  `EPIC-M3.5: materiality filter narrows the visible feed`,
  `EPIC-M3.5: type filter fetches only the selected source`.
