...
split-track chain. EPIC-M1.141 (API) and EPIC-M1.142 (UI) already
...
- `GET`/`PUT /api/v1/preferences` (`api/routers/preferences.py`,
  ...
  since M1.141's names are the real, already-shipped, tested contract
...
- `POST /api/v1/recommendations/{recommendationId}/feedback`
  ...
  *active* version (409 `MRA_STALE_PREDICTION_VERSION` on mismatch),
  ...
  appended via M1.52's `submit_feedback` (immutable after creation,
...
- All 8 of this EPIC's ACs bullet-for-bullet: preferences-vs-prediction-
  ...
  either the API or UI surface -- all already true of the M1.141/M1.142
...
- `api/schemas/feedback.py::FeedbackHistoryItem` -- `feedbackId`,
  ...
  is realized as `type`, reusing M1.141's exact submission vocabulary,
...
- `api/services/feedback.py::get_feedback_history` -- lists the caller's
  own `RecommendationFeedback` rows (M1.52's `get_feedback_for_user`),
...
- Flutter: `flutter_app/lib/features/feedback/{feedback_history_item,
  ...
  on the existing `PreferencesSettingsScreen` (alongside M1.142's
...
# 20 passed (15 pre-existing M1.141 tests unchanged + 5 new
...
- No new authentication mechanism -- `GET /feedback/history` reuses the
  ...
  (EPIC-M1.145) every other per-user endpoint in this API already uses.
