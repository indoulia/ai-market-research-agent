...
autonomous/epic-m1-67, branched cleanly from `main` (the declared dependency -- M1.66 -- is already merged).
...
`detect_model_regression` compares one model version's own real-world success rate between two strictly disjoint windows -- a baseline and a monitoring window -- reusing M1.25's `EvaluationWindow`/`OverlappingEvaluationWindowsError` (the same abstraction M1.29/M1.30/M1.40/M1.41/M1.43/M1.49 already use) and M1.30's own `REGRESSION_MARGIN`. This is a genuinely different comparison shape than M1.30/M1.43 (which compare two *different* models/candidates): here both windows measure the *same* model version, isolating whether its own real-world performance has degraded over time.
...
`REGRESSION_MARGIN` (reused from M1.30, not redefined) is the explicit degradation threshold (AC: "regression thresholds are explicit"). Both the overall verdict and every per-segment (horizon/regime) breakdown require `MIN_SAMPLE_SIZE_FOR_COMPARISON` (M1.16) on *both* sides before anything other than `INSUFFICIENT_SAMPLE` is possible -- a segment below the floor on either side is simply omitted from `segment_regressions`, never used to draw an unsafe conclusion (AC: "small samples do not trigger unsafe conclusions").

### Segmentation Where Sample Sizes Permit

`_segment_regressions` breaks the comparison down by horizon and by market regime (M1.26's `classify_market_regime`, reused unchanged) -- "where sample sizes permit" (scope), each segment independently gated by the same evidence floor as the overall check.
...
- `docs/epics/EPIC-M1.67-model-regression-detection.md` — this completion report.
...
- [x] Regression thresholds are explicit (`REGRESSION_MARGIN`, reused from M1.30).
