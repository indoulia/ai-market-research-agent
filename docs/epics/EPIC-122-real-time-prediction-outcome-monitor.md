# EPIC-122 — Real-Time Prediction Outcome Monitor
...
## Dependencies
EPIC-042, EPIC-079, EPIC-081, EPIC-095, EPIC-098, EPIC-121.
...
- `app/prediction_outcome_monitor.py` (RTM-001): `evaluate_prediction_realtime` incrementally
  ...
  independent of `app.outcomes.evaluate_recommendation` (EPIC-005), which deliberately only
  ...
  - Absolute target/stop prices come from EPIC-042's `RecommendationPublication` when the
  ...
  - Assumption invalidation reuses EPIC-112's `assess_assumption_decay` read-only: a
    `MATERIAL_DECAY` verdict with `invalidation_recommended=True` closes the prediction here as
    `INVALIDATED`, evidenced by the exact `AssumptionDecayAssessment.id`. EPIC-112 itself has "no
  ...
  - Missing/stale price data during a live trading session (per EPIC-121's `is_trading_session`)
  ...
  - `detect_material_movement` is a pure, stateless signal (not a state transition) flagging an
    ...
    as the condition an orchestrated `OPERATION_PREDICTION_MONITORING` trigger (EPIC-121) would
    check before firing re-analysis; wiring an actual scheduled caller is left to EPIC-121's own
  ...
  - `get_terminal_event`/`get_event_history` are the read surface offered for usefulness,
    ...
    EPIC-125/EPIC-132/EPIC-133), wiring a specific downstream consumer (API/UI/tracking) to actually
