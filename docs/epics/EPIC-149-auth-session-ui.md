# EPIC-149 — Authentication & Session UI
...
## Scope
- Sign-in/session restoration flow using EPIC-148.
...
## Parallelization
UI team against EPIC-148 mock contract.

## Dependencies
EPIC-136, EPIC-137, EPIC-148.
...
### What was built
- `lib/core/auth/auth_session.dart` — `AuthSession` parsed from EPIC-148's real, merged `SessionResponse`/`UserContext` shape (`sessionToken`, `userId`, `issuedAt`, `expiresAt`), with an `isExpired` getter and JSON round-trip for local persistence.
...
- `AuthRepository.refresh()` exists (mirrors EPIC-148's `POST /auth/session` refresh contract) but nothing calls it — there is no proactive "refresh before expiry" background timer. Sessions are re-validated on cold start (`restore()`) and reactively the moment any request returns `MRA_SESSION_EXPIRED` (see the `onSessionExpired` hook above); there is no additional periodic check while the app sits idle and connected.
- `/dev/gallery` is now behind the same auth gate as every other route when a real `AuthController` is wired in (i.e. in the running app, not in tests that omit `authController`). This is a real behavior change from EPIC-136's originally ungated gallery link — treated as correct rather than worked around, since gating "every route except sign-in/splash" is the simpler, more honest contract than special-casing one QA-only route to bypass auth.
...
- Full existing suite (`flutter test`) re-run after every change in this epic: 99/99 passing, zero regressions to EPIC-136/EPIC-137/EPIC-139/EPIC-141/EPIC-143/EPIC-145/EPIC-146's tests.
