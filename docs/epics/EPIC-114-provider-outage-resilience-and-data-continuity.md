# EPIC-114 — Provider Outage Resilience & Data Continuity
...
- Fail over through EPIC-094 provider routing.
...
## Dependencies
EPIC-090, EPIC-094, EPIC-101, EPIC-105.
...
- **Detect provider health degradation / fail over through EPIC-094 provider routing / recover automatically when providers return to healthy state:** already hold unchanged via EPIC-094's own `select_provider`, which recomputes fresh from EPIC-093 on every call and recovers with no configuration change — not duplicated here.
- **Track partial data availability explicitly:** `record_outage_snapshot` classifies a data type's severity as `NONE`/`PARTIAL`/`TOTAL` by reusing EPIC-093's own `ProviderQualityMetric.verdict` unchanged — a provider with `VERDICT_INSUFFICIENT_SAMPLE` or no metric at all is honestly counted as healthy, not degraded, the same "insufficient sample is not the same as unreliable" posture EPIC-094's own selection logic already established (verified by `test_insufficient_sample_not_treated_as_degraded` and `test_provider_with_no_metric_at_all_treated_as_healthy`).
...
- **Prevent stale provider data from being treated as current / suppress affected predictions when minimum evidence policy is not satisfied:** already covered by EPIC-030's freshness checks, EPIC-078's evidence-quality gate, and EPIC-112's assumption-decay tracker — not duplicated; this module's own signal is a read-only input a future revision of those could compose, with no write path to `Prediction` or any recommendation-facing table.
