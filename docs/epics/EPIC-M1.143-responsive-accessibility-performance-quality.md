...
- `MraChip` (EPIC-M1.133, used by essentially every screen): its label `Text` had no `Flexible`/ellipsis, so a chip placed inside any tightly-constrained parent (an `Expanded`/`Flexible` slot) overflowed at 2x text scale. Fixed in the shared component itself — every chip usage across the app benefits, not just the two sites that surfaced it in testing.
