...
- **Compare prediction performance against peer-relative context:** `compare_sector_performance` reuses M1.11's/`trust_report`'s own `VERDICT_OK`/`VERDICT_WEAK`/`VERDICT_INSUFFICIENT_SAMPLE` vocabulary and `WEAKNESS_MARGIN`, comparing a sector's success rate against the platform-wide baseline within a window — the same always-fresh "report" posture as M1.85/M1.99/M1.102/M1.108.
