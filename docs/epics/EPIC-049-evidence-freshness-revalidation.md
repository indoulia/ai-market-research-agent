# EPIC-049 — Evidence Freshness & Revalidation

**Status:** DONE  
**Execution Status:** COMPLETED  
**Priority:** P1  
**Dependency:** EPIC-030, EPIC-043
...
## Dependency Chain
EPIC-030/EPIC-043 → EPIC-049 → EPIC-050
...
- `docs/epics/EPIC-049-evidence-freshness-revalidation.md` — this completion report.
...
- `pytest -q tests/test_evidence_revalidation.py -v`: **9 passed** — the horizon-aware threshold never shrinks below EPIC-030's base policy; fresh evidence requires no revalidation; stale, missing, and changed evidence each correctly trigger revalidation with the right reason; a longer horizon tolerates more staleness than the flat EPIC-030 threshold would allow; a revalidation check never mutates the original EPIC-043 snapshot; a check row is immutable after creation; the full revalidation history retains every check performed.
...
- [x] Each evidence category has an explicit freshness policy (mapped to EPIC-030's per-data-type base thresholds, reused unchanged).
...
- [x] Stale evidence is visible to users and downstream scoring (queryable via `get_revalidation_history` and EPIC-043's own `is_stale`/`status`).
...
I believe this implementation satisfies all six acceptance criteria with real, verified evidence, including a real-Postgres migration round-trip and a direct proof that a longer horizon genuinely tolerates more staleness than EPIC-030's flat threshold would. This EPIC generalizes EPIC-030's freshness policy to be horizon-aware without modifying EPIC-030 itself, and never touches EPIC-043's own immutable snapshot table. Per the user's standing-contract update, Claude will merge this PR once CI is green and it is cleanly mergeable, then continue to the next eligible EPIC.
