...
autonomous/epic-m1-10 (stacked on the still-open `autonomous/epic-m1-8` branch/PR #21, since EPIC-010 depends on EPIC-008 and it hasn't merged yet — sibling to EPIC-009's branch, not built on top of it)
...
- **Persistence + enforcement:** `record_recommendation_with_selected_horizon(session, consensus_evaluation, atr_percent, **kwargs)` selects the horizon first, then routes through EPIC-008's `record_qualifying_recommendation` (so a non-qualifying candidate still can't be recorded) with `horizon_days` and the new `horizon_selection_version` traced. Added `Prediction.horizon_selection_version` (new, required, added to `IMMUTABLE_FIELDS`) via migration `0009_horizon_selection_version`, chaining off `0008_consensus_contract_version` — the existing `horizon_days` field (EPIC-004) was already immutable, so "cannot be silently changed after issuance" was already true for the horizon value itself; this EPIC adds traceability for *which rule* chose it.
...
This branch and EPIC-009's branch (`autonomous/epic-m1-9`, PR #22) are siblings — both stacked on `autonomous/epic-m1-8`, neither depends on the other. Both independently added a new migration numbered `0009` chaining off `0008_consensus_contract_version` (EPIC-009: `0009_opportunity_score`; this EPIC: `0009_horizon_selection_version`). Whichever PR merges **second** will hit the same migration-history collision already documented and resolved once before (EPIC-012 vs. EPIC-005) — two revisions both claiming `down_revision=0008_consensus_contract_version`. Resolution depends on merge order, which is outside Claude's control (Claude does not merge PRs); the second-merged one will need renumbering to `0010` with an updated `down_revision`, then re-validation. Flagged here rather than resolved, per the same precedent.
...
- `docs/epics/EPIC-010-horizon-selection.md` — this completion report.
...
- `pytest -q`: **73 passed**, 4.20s (56 pre-existing/EPIC-008 + 17 new in `test_positive_horizon_selection.py`).
