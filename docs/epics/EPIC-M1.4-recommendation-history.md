...
Persist every positive recommendation exactly as issued, with a unique ID, immutable core fields, and query support by symbol and time range, so it can be evaluated later (M1.5) without being retrospectively altered.
...
- `docs/M1-STATUS.md` — reflects Yahoo provider and M1.4 as implemented; next task is M1.5.
- `docs/epics/EPIC-M1.4-recommendation-history.md` — this completion report.
...
- AC-7 "No outcome-success calculation is included; that belongs to M1.5": PASS. Evidence: `app/recommendations.py` contains no success/outcome/return-evaluation logic; `status` always starts `"OPEN"` and is left for M1.5 to transition.

### Validation

Ran the actual local test suite (Python 3.10 interpreter with `requirements.txt` installed) rather than only `compileall`, unlike the prior autonomous runs for M1.2/M1.3 which could not execute `pytest` in their sandbox. Also independently validated the new Alembic migration applies and reverses cleanly against a disposable scratch PostgreSQL database, which prior EPICs in this repo had not done.
...
- **Pre-existing broken migration, unrelated to this EPIC**: `migrations/versions/0003_market_price_dedupe.py` calls `op.create_index("uq_market_prices_stock_timestamp", ...)`, but `migrations/versions/0001_initial.py` already creates a `UniqueConstraint` with the exact same name on `market_prices` (which Postgres backs with an identically-named index). Running `alembic upgrade head` against **any fresh database** fails at `0003` with `psycopg.errors.DuplicateTable: relation "uq_market_prices_stock_timestamp" already exists` — this is not local drift, it reproduces on a brand-new empty database. `0003`'s `downgrade()` is similarly broken (drops an index it never successfully created). This likely explains why CI never attempts to run migrations. Recommend a follow-up EPIC/fix: either drop the redundant `0003` migration or rename its index. Not fixed here — out of scope for M1.4 and unrelated to recommendation history.
...
- The M1 foundation scaffold (`predictions`, `prediction_outcomes`, `model_versions` tables) was already shaped almost exactly for M1.4/M1.5's needs before either EPIC was written. M1.5 (Evaluate Recommendation Outcomes) should very likely build on the existing `PredictionOutcome` model in the same way — extending it minimally rather than introducing a parallel table — and should transition `Prediction.status` from `"OPEN"` to something evaluated (e.g. `"EVALUATED"`), which this EPIC's immutability listener already accommodates.
- `get_recommendation_history` returns ORM `Prediction` objects directly; if M1.6 (performance reporting) needs aggregate queries across many recommendations, consider adding purpose-built aggregate query functions then rather than materializing full ORM object lists.
