# EPIC-051 — Adaptive Recommendation Adjustment

**Status:** DONE  
**Execution Status:** COMPLETED  
**Priority:** P1  
**Dependency:** EPIC-071, EPIC-072, EPIC-024, EPIC-036, EPIC-048, EPIC-050
...
## Dependency Chain
EPIC-071/EPIC-072/EPIC-024/EPIC-036/EPIC-048/EPIC-050 → EPIC-051 → EPIC-052
...
autonomous/epic-m1-56, branched cleanly from `main` (all six declared dependencies -- EPIC-071, EPIC-072, EPIC-024, EPIC-036, EPIC-048, EPIC-050 -- are already merged).
...
- **`PROBABILITY_CALIBRATION`** — EPIC-024's `build_calibration_candidate`, flagging `OVERCONFIDENT`/`UNDERCONFIDENT` probability buckets, validated strictly out-of-sample via EPIC-024's own `evaluate_calibration_candidate_out_of_sample`.
- **`REGIME_SCORE_ADJUSTMENT`** — EPIC-036's `build_regime_score_adjustment_candidate`, flagging miscalibrated regimes, validated via EPIC-036's own `evaluate_regime_score_adjustment_out_of_sample`.
- **`FEEDBACK_LEARNING_SIGNAL`** — EPIC-048's `compute_feedback_learning_signals`, flagging `WEAK` feedback category/reason patterns. These have no out-of-sample validation mechanism of their own, so they are always surfaced `PENDING` rather than a fabricated validation -- an honest limitation, not an oversight (AC: "candidates with insufficient evidence are ... marked pending").
...
- `docs/epics/EPIC-051-adaptive-recommendation-adjustment.md` — this completion report.
...
- [x] Historical replay compares baseline and candidate (reuses EPIC-024/EPIC-036's own out-of-sample comparisons).
