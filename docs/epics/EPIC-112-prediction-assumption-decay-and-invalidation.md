# EPIC-112 — Prediction Assumption Decay & Invalidation
...
## Dependencies
EPIC-060, EPIC-105, EPIC-106, EPIC-110.
...
- **Track assumptions supporting each prediction:** every EPIC-043 evidence category that was `STATUS_AVAILABLE` at capture time is one tracked assumption (`FUNDAMENTAL`, `NEWS`, `EVENT`, `TECHNICAL_VOLUME`). `MARKET_SECTOR` is honestly excluded — a static classification with no real freshness-window concept in EPIC-030's own policy, named in the docstring rather than assigned an invented threshold.
- **Define assumption freshness/decay rules:** reuses EPIC-030's own `FRESHNESS_POLICY` thresholds unchanged, but applies them against the *original* `evidence_timestamp` EPIC-043 froze at capture, compared against `evaluated_at` (now) rather than the prediction's own `as_of_timestamp` (then) — a genuinely new check EPIC-078's evidence-quality gate never performs (that gate only ever asks whether evidence was fresh *at capture*).
- **Detect material contradiction or thesis break / trigger revalidation or invalidation:** `decay_ratio` is the fraction of tracked categories that have crossed their freshness window since capture; `MATERIAL_DECAY` (`decay_ratio >= 0.5`) sets `invalidation_recommended=True`. Propose-only — no write path to `Prediction` or any recommendation-facing table; an actual revalidation/revision remains EPIC-057/EPIC-050/EPIC-105's job.
