# EPIC-033 — Objective Recommendation Outcome Measurement
...
autonomous/epic-m1-38, branched cleanly from `main` (both declared dependencies -- EPIC-031 and EPIC-032 -- are already merged).

### Objective

Determine recommendation success/failure/neutral/insufficient-data using predefined, immutable, versioned rules, without touching EPIC-005's foundational outcome computation.

### Design Decisions

- **Deliberately does not modify `app/outcomes.py` (EPIC-005).** That module is the foundational, heavily-tested source of the objective per-recommendation result (target/stop-hit detection, realized return at the selected horizon, invalid-data handling) that a large fraction of this platform's already-merged EPICs (EPIC-018, EPIC-070, EPIC-072, and every analysis EPIC downstream) depend on and are tested against. Modifying its core classification logic now would be genuinely risky for no real benefit.
- **What EPIC-005's `PredictionOutcome` actually lacks -- and what this EPIC adds** -- is a traceable classification *version* (it has none at all) and an explicit `NEUTRAL` category. EPIC-005's own fallback path (no threshold hit) classifies purely by the sign of `actual_return`, even when that return is negligibly close to zero; the scope explicitly names `NEUTRAL` as a fourth first-class outcome alongside success/failure/insufficient-data, which nothing in this repo currently produces.
- **New table `outcome_measurements`** (migration `0027`, chains off EPIC-032's `0026`): one immutable row per `PredictionOutcome` (unique `prediction_outcome_id`), never updated after creation (`before_update` guard). Every other classification EPIC-005 already determined (target hit → `SUCCESS`, stop hit → `FAILURE`, invalid data → EPIC-033's `INSUFFICIENT_DATA`, explicitly renamed from EPIC-005's `UNEVALUABLE` to match this EPIC's own named vocabulary) passes through completely unchanged; only the narrow "no threshold hit, return near zero" case is reclassified.
...
- **"Missing data never becomes an assumed success/failure" (AC)** holds because `UNEVALUABLE` maps to `INSUFFICIENT_DATA` unconditionally, before any return-based classification is even considered, and `realized_return` is explicitly `None` for that case rather than echoing EPIC-005's placeholder `actual_return=0`.
...
- **"Freeze final outcome after sufficient evidence exists" (scope)** holds structurally: `measure_outcome` only ever accepts an already-persisted `PredictionOutcome`, which itself only exists once EPIC-005 has completed evaluating the full horizon window -- there is no code path to measure an in-progress recommendation.
...
- `docs/epics/EPIC-033-objective-outcome-measurement.md` — this completion report.
