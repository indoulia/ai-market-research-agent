# EPIC-084 — Horizon & Regime-Specific Trust
...
## Dependency Chain
**Previous:** EPIC-080, EPIC-081, EPIC-021.
**Next:** EPIC-085, EPIC-082, EPIC-087.
...
autonomous/epic-m1-79, branched cleanly from `main` (the declared dependencies -- EPIC-080, EPIC-081, EPIC-021 -- are already merged).
...
This platform only ever generates a real `Prediction` for a positive (upward-trending) candidate (EPIC-009/EPIC-016's "positive-only" qualification), so a `BEARISH_*` regime segment can never have real evaluated evidence here -- the same honest, forward-compatible constraint already established for EPIC-010's never-populated horizon day 2 and EPIC-041's MEDIUM/LONG bands. `test_a_different_regime_is_isolated` instead proves cross-segment isolation using two regimes that genuinely can both occur for positive candidates (differing volatility bands).

### Feeds A Future Gate, Enforces Nothing Itself

`is_low_trust` is exposed for a future gate (EPIC-082, not yet built) to consume -- this module has no write path to `Prediction`/`ScanCandidate`/any selection table, matching this platform's established propose/gate split (EPIC-060, EPIC-078, EPIC-080).
...
- `docs/epics/EPIC-084-horizon-regime-specific-trust.md` — this completion report.
...
I believe this implementation satisfies all five acceptance criteria with real, verified evidence, including a real-Postgres migration round-trip and a hand-verified exact statistical result for a sufficient-sample segment. This EPIC composes EPIC-021's regime classification and the same evaluated-prediction history EPIC-080 reads, without duplicating EPIC-080's own blended composite, and never enforces suppression itself -- only makes the signal available for a future gate. Per the user's standing-contract update, Claude will merge this PR once CI is green and it is cleanly mergeable, then continue to the next eligible EPIC.
