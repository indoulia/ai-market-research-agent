# EPIC-165 — Application E2E Contract Validation
...
### What already existed (EPIC-147 + per-screen EPICs)

This EPIC's scope overlaps heavily with **EPIC-147** ("Flutter/API
Integration & End-to-End Contract Validation", the *old* split UI/API
track — distinct from `EPIC-164`, which is the *combined-roadmap*
epic that inherited the "144" number before the M3.x renumbering; see
`docs/epics/EPIC-M3-ROADMAP-NOTE.md`). EPIC-147, already merged, built:
...
does **not** duplicate: EPIC-153 dashboard (`dashboard_screen_test.dart`),
EPIC-154 explorer (`opportunity_explorer_screen_test.dart`), EPIC-155 detail +
...
EPIC-156 news/events (`news_events_screen_test.dart`), EPIC-157 discovery
(`discover_screen_test.dart`, `discovery_pipeline_panel_test.dart`),
EPIC-159 active-position monitoring incl. target/SL/status
(`tracking_screen_test.dart`), EPIC-160 learning
(`learning_repository_test.dart`, `learning_screen_test.dart`), EPIC-161
feedback/preferences (`feedback_history_screen_test.dart`,
`*preferences*_test.dart`), EPIC-162 system health
(`system_health_screen_test.dart`), EPIC-163 auth
(`sign_in_screen_test.dart`, `app_router_auth_test.dart`), and EPIC-164
...
had fallen behind EPIC-153/EPIC-154/EPIC-155/EPIC-157/EPIC-159/EPIC-160/EPIC-161 — a real,
...
`ApiClient.debugHttpClientOverride` scripting as EPIC-147's suite; real
...
- `flutter_app/test/e2e/news_to_prediction_journey_test.dart` (395
  ...
  first time.
- **Journey 1** (Login → Home) was already fully covered by EPIC-147's
  suite; not duplicated.
- **Journey 4** (Detail → Feedback) was already covered end-to-end by
  EPIC-147's happy-path test (detail → feedback → preferences chain).
...
pre-existing 5 EPIC-147 E2E tests, the 3 new `cross_screen_journeys_test.dart`
...
- **Journeys 6 ("Active prediction → target/SL update"), 7 ("Trust
  ...
  the (real-repository-contract, fake-transport) screen level by EPIC-159's
