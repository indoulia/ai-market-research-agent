...
- Composite score: fixed weighted average of expected return (`Prediction.target_return`, normalized against a 20% cap), calibrated probability (`Prediction.predicted_probability`), M1.77 trust (`PredictionTrustScore.overall_trust_score`), M1.74 evidence quality (1.0 when `STATE_SUFFICIENT`), plus M1.47 reward/risk (`RecommendationPublication.reward_risk_ratio`) and M1.83 stability (`PredictionStabilityAssessment.stability_verdict`) when available — weights renormalize over whichever of the two optional signals are present, so a candidate is never penalized for a signal nobody has computed yet.
...
**Not wired into any live selection/recommendation feed** — consistent with this platform's established propose/gate split (M1.65/M1.74/M1.77/M1.79/M1.80/M1.81 are all likewise available-but-not-wired); wiring ranking into the actual feed remains a future EPIC's job, same posture M1.81 itself documented for M1.84.
