# EPIC-030 — Automatic Information Refresh Policy
...
autonomous/epic-m1-35, branched cleanly from `main` (declared dependency EPIC-029 is already merged).
...
- `docs/epics/EPIC-030-information-refresh-policy.md` — this completion report.
...
- [x] Stale data triggers refresh or explicit non-qualification (explicit `stale_data`/`missing_data` reasons; downstream callers -- e.g. EPIC-015's scan -- already treat missing/stale market data as explicit non-qualification, consistent with this module's classification).
