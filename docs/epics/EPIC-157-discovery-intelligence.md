# EPIC-157 — Discovery Intelligence
...
(`ac21500`, EPIC-154 merged). Re-checked immediately before opening the
PR: still neither branch exists on `origin`, and `origin/main` had not
moved past `ac21500`, so this PR carries only EPIC-157's own commits —
no dependency on EPIC-155/EPIC-156 in practice.

**Context:** per the roadmap note (`docs/epics/EPIC-M3-ROADMAP-NOTE.md`)
and the pattern already established by EPIC-152–EPIC-156, this session's job was
to diff EPIC-157's scope against the existing, merged discovery/qualification
pipeline (EPIC-015/EPIC-016/EPIC-020/EPIC-068/EPIC-023/EPIC-142) — not to build a
second, parallel discovery API or screen. Unlike its siblings, EPIC-157's
...
- The discovery/qualification/publication data model this EPIC needs to
  make visible already exists in full: `DiscoveryRecord` (EPIC-015),
  `ScanCandidate`/`RecommendationGeneration` (EPIC-016, consensus
  qualify/reject with `failed_criteria`), `PositiveOpportunityRanking`
  (EPIC-068, `included`/`exclusion_reason`), `RecommendationSelection`
  (EPIC-068, `selected`) — `app/models.py`.
- `app.discovery_effectiveness.compute_discovery_effectiveness_report`
  (EPIC-023) already computes the exact per-source discovered/routed/
...
- `api/services/segmentation.py` (`market_cap_bucket_expr`,
  ...
  unchanged.
- `GET /api/v1/discoveries` (EPIC-142) already served most of the raw
...
- `api/deps.get_optional_bearer_subject`/`AuthSession`/
  `get_session_status` (EPIC-148) — the session-validation primitives
...
- `GET /api/v1/discovery/candidates` — did not exist under this path.
  ...
  reinterpretation EPIC-154 already established for `/opportunities`),
  ...
  mechanism EPIC-148 already ships rather than inventing a new
...
- `GET /api/v1/discovery/summary` — did not exist. Funnel counts
  ...
  `api/schemas/discovery.py`). Plus `effectivenessBySource` (EPIC-023's
...
- `discover_screen.dart` (EPIC-143's existing, well-tested Discover screen)
...
- No role/permission system was added for the "internal/authorized"
  ...
  a live EPIC-148 session", which is what every real app user already
...
- `/discoveries` (EPIC-142) was left completely untouched, still passing
