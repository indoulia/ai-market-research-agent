...
autonomous/epic-m1-23, branched cleanly from `main` (both declared dependencies, M1.22 and M1.16, are already merged).

### Objective

Measure whether M1.13's reported `predicted_probability` corresponds to realized M1.5 outcomes -- true calibration -- across horizons, without ever rewriting a historical confidence value.

### Design Decisions

- **No new table or migration.** Read-only aggregation over `Prediction`/`PredictionOutcome`, exactly like M1.6/M1.16/M1.22.
- **Genuine calibration, distinct from M1.16's weak-segment detection.** M1.16 flags a bucket performing *below the platform's overall success rate*; this EPIC instead compares each bucket's *own* average stated `predicted_probability` against its *own* observed success rate (`calibration_error = average_predicted - observed`) -- the actual definition of calibration, and a materially different question from M1.16's. Reuses M1.6's ten fixed-width probability buckets (`PROBABILITY_BUCKET_COUNT`/`PROBABILITY_BUCKET_WIDTH`, both public) rather than redefining bucket boundaries a third time.
...
- `docs/epics/EPIC-M1.23-recommendation-confidence-analysis.md` — this completion report.
