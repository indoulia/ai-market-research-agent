...
autonomous/epic-m1-61, branched cleanly from `main` (the declared dependency -- M1.60 -- is already merged).
...
This platform's horizon selection (M1.10) only ever produces short-term (1-7 day) predictions today; "multi-horizon" in practice means comparing several currently-open predictions for the same stock made at different times with different M1.10-selected horizons (1, 3, 5, or 7 days). `get_horizon_views` preserves each one's own already-immutable score/confidence (`Prediction`) and horizon independently (scope: "preserve horizon-specific scores, confidence, target, and SL" -- target/SL remain queryable per prediction via M1.47, not duplicated here).
...
Reuses M1.46's `UserPreference.horizon_band`/`custom_horizon_days` directly: among currently-open predictions matching the user's preferred band, the highest-scoring one is chosen as primary; if none match, the single best-scoring prediction overall is still surfaced rather than returning nothing (scope: "never hide a material conflicting view"). Because this composes M1.46's own preference system, "default user horizon remains short term" and "other horizons are opt-in/configurable" (AC) are inherited for free -- M1.46 already established `SHORT` as the default and `MEDIUM`/`LONG`/`CUSTOM` as explicit opt-ins.
...
- `docs/epics/EPIC-M1.61-multi-horizon-resolution.md` — this completion report.
...
- [x] Default user horizon remains short term (1-7 days) (inherited from M1.46's `DEFAULT_HORIZON_BAND = SHORT`, proven by test).
- [x] Other horizons are opt-in/configurable (inherited from M1.46's `MEDIUM`/`LONG`/`CUSTOM` bands; proven by test with a custom-horizon preference).
...
I believe this implementation satisfies all four acceptance criteria with real, verified evidence, including a real-Postgres migration round-trip and direct proof that a user's explicit preference can override the default and still honestly surface the outvoted view as a conflict. This EPIC composes M1.46's preference system and M1.47's already-immutable per-prediction scoring/target-SL without duplicating either, and never fabricates coverage for horizon bands (medium/long) this platform's own pipeline has never populated. Per the user's standing-contract update, Claude will merge this PR once CI is green and it is cleanly mergeable, then continue to the next eligible EPIC.
