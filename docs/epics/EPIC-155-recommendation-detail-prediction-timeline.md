# EPIC-155 — Recommendation Detail & Prediction Timeline
...
- Target/SL/outcome states are consistent with EPIC-122.
...
the PR: PR #260 (and #261, EPIC-153) were both still open. This EPIC's
scope does not depend on either — it only touches `/api/v1/recommendations/*`
and `flutter_app/lib/features/detail/*`, neither of which EPIC-153/EPIC-154 modify —
...
strip EPIC-154's own commits back out, leaving only this EPIC's changes
...
`EPIC-138-recommendation-detail-prediction-timeline.md`. The real, substantive
overlap is with **`EPIC-140` (Recommendation Detail, Revision & History
API, `DONE`, merged PR #166) and `EPIC-141` (Recommendation Detail &
...
job was to diff EPIC-155's specific scope against that existing, merged code,
not to build a second, parallel screen or API.

**Already satisfied by existing EPIC-140/EPIC-141 work — verified, reused,
not reimplemented:**
...
- Header, metric grid (horizon/target/SL/upside/score/confidence/trust),
  ...
  (EPIC-141) already built all of these against EPIC-140's contracts.
...
- Immutability of historical revisions — `app/recommendation_revision.py`'s
  `RecommendationRevisionImmutableError` guard (EPIC-050), unchanged; this
...
- `GET /api/v1/recommendations/{recommendationId}/timeline` — did not
  exist under any name. EPIC-140's `/history` is revisions-only (a
...
- `GET /api/v1/recommendations/{recommendationId}/evidence` — did not
  ...
  `/{id}`'s detail payload). Following the same precedent as EPIC-152
...
- `RecommendationDetail.evidenceFreshness` — new field on the detail
  ...
  EPIC-147). EPIC-140's detail response never exposed this, even though
...
- `recommendation_detail_screen.dart` — the screen now fetches `/timeline`
  ...
  - Header now shows a "Stale evidence" badge when `evidenceFreshness ==
    ...
    convention (EPIC-147) of only ever asserting confirmed staleness, never
  ...
  - Progressive disclosure: new `flutter_app/lib/design_system/components/mra_expandable_section.dart`
    ...
    remains uncluttered" line, which EPIC-141 had not implemented (it only
...
- No second Flutter screen or second API surface — EPIC-140/EPIC-141's
...
- Benchmark-relative result, true real-time outcome-detection latency and
  trading-day-aware `expiryAt` remain the same named, honest gaps EPIC-140
  already documented (EPIC-132/EPIC-122/EPIC-124 respectively) — this EPIC
  neither claims nor fabricates progress on them.

**Conclusion:** EPIC-155's scope was already ~85% satisfied by the
existing, merged EPIC-140/EPIC-141 work. The concrete, named gaps — a
