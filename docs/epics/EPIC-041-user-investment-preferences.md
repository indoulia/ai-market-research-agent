# EPIC-041 — User Investment Preferences
...
autonomous/epic-m1-46, branched cleanly from `main` (the declared dependency -- EPIC-017 -- is already merged).

### Objective

Let each user define investment preferences that control which of EPIC-017's already-selected recommendations are surfaced to them, without changing EPIC-009's scoring, EPIC-010's horizon selection, EPIC-016's generation, or EPIC-017's system-wide daily selection in any way.

### Design

Two append-only tables:
- **`UserPreference`**: versioned per user -- the same "the log is the pointer" pattern EPIC-026/EPIC-039 already established. The most recent row for a `user_id` is that user's current effective preference; changing a preference (`set_user_preference`) always inserts a new row, never updates a prior one.
...
- `docs/epics/EPIC-041-user-investment-preferences.md` — this completion report.
...
I believe this implementation satisfies all six acceptance criteria with real, verified evidence, including a real-Postgres migration round-trip and a direct proof that a preference change never alters an already-recorded snapshot. This EPIC never modifies EPIC-009/EPIC-010/EPIC-016/EPIC-017 -- it composes EPIC-017's own selection output and EPIC-029's market-cap bucket vocabulary into a purely additive, per-user personalization layer. Per the user's standing-contract update, Claude will merge this PR once CI is green and it is cleanly mergeable, then continue to the next eligible EPIC.
