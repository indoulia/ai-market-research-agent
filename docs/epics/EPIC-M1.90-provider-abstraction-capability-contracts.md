...
autonomous/epic-m1-90, branched cleanly from `main` (the declared dependencies -- M1.72, M1.73, M1.35 -- are already merged).
...
- `docs/epics/EPIC-M1.90-provider-abstraction-capability-contracts.md` — this completion report.
...
I believe this implementation satisfies all five acceptance criteria with real, verified evidence, including a direct proof that swapping a concrete provider requires zero changes to the domain ingestion functions that consume it. This EPIC retrofits M1.3/M1.72/M1.73's already-built adapters additively rather than rewriting any of them, and honestly declares the AI/discovery capability contract-only where this platform has no real implementation, matching the same posture M1.35 held for fundamentals/news before real pipelines existed. Per the user's standing-contract update, Claude will merge this PR once CI is green and it is cleanly mergeable, then continue to the next eligible EPIC.
