...
autonomous/epic-m1-16, branched cleanly from `main` (both declared dependencies, M1.6 and M1.15, are already merged).

### Objective

Expose the historical truth of recommendation performance -- reusing M1.6's deterministic performance aggregates in full -- and additionally flag which 1/3/5/7-day horizons and predicted-probability buckets are performing weakly relative to the overall success rate, but only when each has enough evaluated samples to support that comparison.

### Design Decisions

- **No new table or migration.** M1.16 adds no new persisted state; it is a pure, deterministic read-side aggregation over M1.6's `compute_performance_report`, which itself reads only `Prediction`/`PredictionOutcome` rows (the same rows M1.15's scheduler populates over time). Reuses `app/performance.py` entirely rather than duplicating any of its logic (scope items 1-6 are already satisfied unchanged by M1.6's existing report).
...
- **Three verdicts, fixed product/policy constants (`TRUST_REPORT_VERSION = "TRUST-001"`):**
  ...
  - Both constants are fixed, documented policy values (mirroring the pattern of `MIN_SCORE_FOR_SELECTION` in M1.14); either would be bumped via `TRUST_REPORT_VERSION` if changed, never silently.
- **Failures/unevaluable stay visible** exactly as M1.6 already reports them (`performance.failure_count`, `performance.unevaluable_count`) — M1.16 adds no filtering or hiding logic on top (non-goal: "hiding or filtering failures to improve presentation").
...
- `docs/epics/EPIC-M1.16-recommendation-trust-report.md` — this completion report.
...
- `git diff --check`
- `alembic heads` (no migration added by this EPIC; head is unchanged from M1.15's `0015_recommendation_lifecycles`)
...
- [x] Every success percentage includes its sample count (inherited unchanged from M1.6's `PerformanceReport`/`HorizonPerformance`/`ProbabilityBucketPerformance`).
...
I believe this implementation satisfies all eight acceptance criteria with real, verified evidence. M1.16 deliberately reuses M1.6's report wholesale rather than reimplementing any of its statistics, since the EPIC's own dependency chain lists M1.6 as providing "the historical performance calculations" — M1.16's only genuinely new contribution is the weak-horizon/weak-bucket verdict layer (scope item 7) and the explicit insufficient-sample distinction it requires. `MIN_SAMPLE_SIZE_FOR_COMPARISON` and `WEAKNESS_MARGIN` are documented design choices open to reviewer adjustment. Per the user's 2026-08-20 standing-contract update, Claude will merge this PR once CI is green and it is cleanly mergeable, then continue to the next eligible EPIC.
