...
Items expose the same canonical recommendation summary contract as EPIC-153.
...
PR: still not pushed, and `origin/main` had not moved. EPIC-153 (Market
Overview & Home Dashboard) was therefore still `APPROVED`/not merged when
this PR was opened — this EPIC does not depend on EPIC-153's dashboard
screen or its `GET /api/v1/dashboard/snapshot` endpoint, only on the
canonical recommendation-summary *shape* EPIC-153's doc also references,
which already exists from EPIC-138 (see below), so no blocking
...
"is" EPIC-154 (it maps `EPIC-154 -> was EPIC-137`, but old `EPIC-137` is "Flutter App
Shell & Responsive Navigation," an unrelated screen). Investigation
found the real, substantive overlap is with **`EPIC-138 -
Recommendations Query API`** (`docs/epics/EPIC-138-recommendations-api-and-query.md`,
`DONE`, merged PR #156) and its Flutter consumers (`EPIC-139` dashboard,
`EPIC-143` discover). This session's job was to diff EPIC-154's specific
...
**Already satisfied by existing EPIC-138/EPIC-139/EPIC-143 work -- verified,
...
- The canonical recommendation-summary contract (`symbol`, `name`/`companyName`,
  ...
  items are "the same canonical recommendation summary contract as EPIC-153."
- The server-side join graph composing EPIC-118's ranking, EPIC-018's
  lifecycle, EPIC-080's trust score, EPIC-019's confidence classification and
  EPIC-042's publication -- `api/services/recommendations.py::list_recommendations`,
...
- `liquidity_bucket_expr`/`market_cap_bucket_expr` SQL expression
  builders (`api/services/segmentation.py`), already built for EPIC-142's
...
- Page-based pagination primitives (`api/pagination.py::PageParams`,
  `MAX_PAGE_SIZE`) existed since EPIC-135 but were **never actually
...
- Flutter design-system pieces already built but **unused anywhere**:
  ...
  built in EPIC-136, zero call sites before this session) is exactly
...
- `GET /api/v1/opportunities` -- did not exist under any name; built new,
  ...
  here since EPIC-138's own reason for avoiding one -- keyset pagination --
...
- New filters: `minUpside` (reusing the existing upside-coalesce
  ...
  EPIC-155/EPIC-159's job), and server-side `search` (`ilike` over
...
- Single-param `sort` with a `-` prefix for descending (e.g. `sort=-score`),
  ...
  described as a "future, not yet needed" extension of EPIC-138's
  `sort`+`direction` pair -- now needed, so adopted and the doc updated
  to describe both conventions and when each applies.
- `marketCap` accepts the same bucket vocabulary as EPIC-138's
...
- `tests/test_api_opportunities.py` (18 new tests): empty feed;
  ...
  EPIC-008's consensus gate requires `volume_ratio_20d >= 0.75`, documented
...
(EPIC-118/EPIC-018/EPIC-080/EPIC-019/EPIC-042 via EPIC-138's join-graph pattern),
segmentation helpers (EPIC-142), page-based pagination primitives
(EPIC-135, previously unused), and several Flutter design-system
components (`MraDenseTable`, `showMraBottomSheet`, EPIC-136) already
