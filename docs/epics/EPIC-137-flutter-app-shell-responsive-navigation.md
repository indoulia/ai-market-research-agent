# EPIC-137 — Flutter App Shell & Responsive Navigation
...
## Parallelization
UI shell team. Can proceed with API team after EPIC-135 contract is established.
...
**Implemented on branch:** `autonomous/epic-m1-133` (same branch as EPIC-136 — the two are tightly coupled UI-foundation pieces implemented and tested together; see that doc's Completion Report for the design-system half).
...
- Settings screen links to the EPIC-136 component gallery, so the QA gallery stays reachable from the running app rather than only from `main.dart`.
