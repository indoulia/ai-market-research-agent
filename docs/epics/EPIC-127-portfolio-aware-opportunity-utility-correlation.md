# EPIC-127 — Portfolio-Aware Opportunity Utility & Correlation
...
## Dependencies
EPIC-118, EPIC-098, EPIC-099, EPIC-109, EPIC-110, EPIC-115.
...
- `app/portfolio_opportunity_correlation.py` (PCR-001/PUA-001/PSE-001):
  - `assess_portfolio_correlation`: for one scan's EPIC-009-qualified candidates, computes real Pearson
  ...
  - `apply_portfolio_adjustment`: layers a deterministic, versioned concentration/correlation
    penalty onto EPIC-118's already-persisted `composite_score` (read via the scan's
    `PositiveOpportunityRanking` rows), optionally cost-scaled by EPIC-098's `ExecutionCostAssessment`
  ...
  - User preference support: reuses EPIC-026's `UserPreference.preferred_sectors` read-only -- a
  ...
  - `measure_portfolio_selection_effectiveness`: compares the diversified/adjusted top-K's
    realized success rate against EPIC-118's raw composite-ranked top-K over the same
    `PredictionOutcome` evidence, mirroring EPIC-099's `measure_ranking_effectiveness` pattern
...
- **Liquidity** beyond EPIC-098's own `liquidity_bucket`/net-return cost-scaling (true order-book
  microstructure) is EPIC-131's domain, not yet implemented on this platform -- not fabricated
  here.
- **Benchmark-relative value**: EPIC-132 (benchmark-relative opportunity alpha) merged during this
  EPIC's implementation but is not a declared dependency of EPIC-127, so it is deliberately not
