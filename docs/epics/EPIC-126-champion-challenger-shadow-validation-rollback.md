# EPIC-126 — Champion/Challenger Shadow Validation & Rollback
...
## Dependencies
EPIC-083, EPIC-119, EPIC-100, EPIC-115.
...
- `app/champion_challenger_shadow.py` (SCS-001/SCC-001/CRB-001):
  ...
  - `execute_rollback`: this EPIC's own genuinely new contribution. `app.
    model_regression_detection` (EPIC-062) already computes `rollback_triggered` but has no
...
- **Usefulness, stability, latency, cost, regime performance** (scope) are each already an
  existing signal elsewhere on this platform (EPIC-061 usefulness, EPIC-083 stability, EPIC-098 cost,
  EPIC-080 regime) but are not yet folded into `ShadowChallengerComparisonReport` -- this EPIC's
  ...
  (EPIC-122, EPIC-127, EPIC-129) documented for their own honest gaps.
