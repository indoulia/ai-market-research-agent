# EPIC-129 — Information Latency & Data Freshness Intelligence
...
## Dependencies
EPIC-090, EPIC-093, EPIC-105, EPIC-121, EPIC-123.
...
- `app/information_latency.py` (ILT-001/LDR-001):
  - `assess_information_latency`: for each `STATUS_AVAILABLE` evidence category (EPIC-043), computes
    decision latency (`Prediction.as_of_timestamp - evidence_timestamp`) and judges it against
    EPIC-030's own `FRESHNESS_POLICY` threshold tightened by `horizon_adjusted_threshold` -- a fixed,
    ...
    wiring an actual gate is left to a future consuming EPIC, the same posture EPIC-112 took before
    EPIC-122 became its first consumer this session. Idempotent by `(prediction_id, evaluated_at)`.
  - `measure_latency_degradation`: compares one window's average ingestion latency
    (`DataFetchAttempt.requested_at - source_timestamp`, EPIC-030) against a baseline window's, per
    data type, mirroring EPIC-099's `measure_ranking_effectiveness` comparison pattern (honestly
