# EPIC-160 — Learning & Self-Improvement
...
- Promotion/rejection states reconcile with EPIC-126.
...
was renumbered from `EPIC-143` in the newer combined roadmap. The older
split-track backend chain (`EPIC-023`-`EPIC-126`) already implements extensive
...
two existed):
- `app.model_promotion.ModelPromotion` (EPIC-026) -- the append-only
  promotion/rejection log this EPIC's AC anchors on ("reconcile with
  EPIC-126"), since `app.champion_challenger_shadow` (EPIC-126) also
  writes into this same table for shadow promotions and rollbacks.
- `app.continuous_learning.LearningCycle` (EPIC-027) -- the
...
- `app.champion_challenger_shadow.ShadowChallengerComparisonReport`/
  `ChampionRollback` (EPIC-126) -- champion/challenger comparison
...
- `app.recommendation_experiments.Experiment`/`ExperimentArm`/
  `ExperimentResult` (EPIC-063) -- the candidate-experiment framework,
  and `app.feedback_experiment_pipeline.FeedbackDrivenExperiment`
  (EPIC-064), which links a recurring feedback pattern to the
  experiment it spawned.
- `app.feedback_learning_signals.compute_feedback_learning_signals`
  (EPIC-048) -- "recent learning signals"/"failure patterns discovered":
...
- A parallel, later generation of the same concepts
  ...
  EPIC-034/EPIC-038/EPIC-039/EPIC-040 dataset-version chain) exists but is
  ...
  The AC's explicit "reconcile with EPIC-126" anchors this EPIC on the
...
- Flutter: `flutter_app/lib/features/learning/{learning_summary,
  ...
  (EPIC-151/EPIC-158), since Learning isn't one of the app's primary
