...
- Also carried in this branch (prerequisite foundation, not itself part of M1.149's scope): the local `docker-compose.yml` postgres/migrate/api/web stack and `MARKET_DATA_PROVIDER=yahoo|upstox` toggle (`scripts/ingest_market_history.py`, `app/settings.py`), plus an `ingest.py` fix for a driver-inconsistent `.rowcount` on `ON CONFLICT DO NOTHING` inserts (now counts `RETURNING` rows instead).
