# EPIC-131 — Market Microstructure & Liquidity Intelligence
...
## Dependencies
EPIC-098, EPIC-099, EPIC-096, EPIC-124.

## Implementation

**Status:** DONE — merged to main via PR #249 (`68606eb`).

`app/microstructure_liquidity.py` reuses `discovery_segmentation.classify_liquidity_bucket` (the same `volume_ratio_20d` thresholds EPIC-029/EPIC-098 already use) and adds the microstructure evidence they didn't capture:

- `compute_average_daily_turnover` — the one liquidity-depth proxy this platform's OHLCV data can honestly support (no real bid-ask spread/order-book depth is ingested anywhere, same honest gap EPIC-098's own docstring names; nothing here fabricates one).
...
- `get_microstructure_snapshot` — the read path a future revision of EPIC-098's execution-cost model could consume (not wired in, same propose-only posture as every other gate module here).
- `compute_liquidity_segment_performance` — segments already-evaluated outcomes by liquidity bucket, gap bucket and circuit-event flag; a superset of EPIC-022's `DiscoverySegment`-based liquidity dimension.
