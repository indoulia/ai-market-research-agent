# EPIC-040 — Continuous Self-Learning Loop
...
- Invoke the EPIC-039 promotion gate.
...
## Dependencies
**Previous:** EPIC-039, EPIC-040 prerequisites EPIC-028–EPIC-039
...
autonomous/epic-m1-45, branched cleanly from `main` (the declared dependency -- EPIC-039 -- and its own prerequisite chain EPIC-028-M1.44 are all already merged).

### The Complete Loop

`run_self_learning_cycle` connects, in order: (1) a watermark-based trigger on `PredictionOutcome.id` (reused unchanged from EPIC-027), (2) EPIC-034's `build_learning_dataset` to (re)build the versioned learning dataset, (3) EPIC-038's `compare_candidate_model` to evaluate the candidate against production on that dataset, (4) EPIC-039's `evaluate_promotion` to gate whether the candidate becomes active, and (5) an optional caller-supplied discovery trigger. Every step composes an existing, already-tested EPIC's public entry point wholesale; this module's only genuinely new logic is the trigger, the wiring between steps, the audit row, and the discovery-trigger hook.

### Scheduling / Trigger Rules

Identical to EPIC-027's watermark mechanism (reused constants `DEFAULT_MIN_NEW_OUTCOMES`, `OUTCOME_RAN`/`OUTCOME_SKIPPED`, `SKIP_REASON_INSUFFICIENT_NEW_EVIDENCE`, imported not redefined): each `SelfLearningCycle` row records the highest `PredictionOutcome.id` it considered; a cycle with fewer new outcomes than `min_new_outcomes` since the last cycle is recorded `SKIPPED` with an explicit reason and never touches the dataset, comparison, or promotion gate (AC: "insufficient evidence causes no change"; scope: "keep the current model when evidence is insufficient"). "The loop runs without manual intervention once enabled" (AC) holds because a caller need only invoke `run_self_learning_cycle` on any schedule (a cron job, a scheduled task) -- the function itself decides whether there is enough new evidence to act on.

### Failure Recovery

"The system can resume safely after interruption" (AC) holds by the same watermark mechanism: the next call to `run_self_learning_cycle` always recomputes `new_outcomes_count` from the last successfully *committed* cycle's watermark, so a crash between cycles never double-counts or loses evidence -- there is no partial-cycle state to recover, only a fresh, correct recomputation. "Failed learning cycles do not stop ordinary recommendation tracking" (AC) holds structurally: this module has no dependency edge from the recommendation-generation pipeline (EPIC-015-M1.14) into it, and no dependency edge from it back into that pipeline other than reading already-immutable `PredictionOutcome` rows -- a failure here cannot propagate there.

### Model Lineage

Every `SelfLearningCycle` row records `dataset_version` (EPIC-034), `comparison_version` (EPIC-038), and `model_promotion_decision_id` (a foreign key into EPIC-039's immutable `model_promotion_decisions` log) -- a complete, queryable chain from one learning cycle back through the exact comparison evidence and dataset version that produced its promotion decision (AC: "active model changes are traceable to comparison evidence"; scope: "preserve a complete lineage from recommendation to model version"). `get_self_learning_cycle_history` returns the full, immutable, chronologically ordered sequence (AC: "every learning cycle has a unique version and audit record").

### Promotion Evidence

This module never recomputes or second-guesses EPIC-039's decision -- it calls `evaluate_promotion` exactly once per cycle and records whatever it returns, promoted or rejected (AC: "no model changes occur without the promotion gate"). There is no code path in this module that could change the active model outside that call.
...
- **New table `self_learning_cycles`** (migration `0030`, chains off EPIC-039's `0029`), deliberately separate from EPIC-027's `learning_cycles` table for the same reason EPIC-039 kept its own table separate from EPIC-026's: the evidence and lineage shape differs structurally (`dataset_version`/`comparison_version`/`model_promotion_decision_id` vs. EPIC-027's `discovery_effectiveness_version`/`calibration_candidate_version`/`candidate_model_evaluation_version`/`model_promotion_id`). EPIC-027's own module and table are left completely untouched.
- **Reuses rather than duplicates**: EPIC-027's watermark trigger constants (imported directly, not redefined), EPIC-034's `build_learning_dataset`, EPIC-038's `compare_candidate_model`/`ModelFunction`, EPIC-039's `evaluate_promotion`.
- **No autonomous trading, no promotion based on training performance alone, no deletion of historical recommendations, no future information for past predictions** (all four non-goals) hold structurally: this module has no trading code path, its only evidence input is EPIC-038's dataset-scoped comparison (itself built from EPIC-034's point-in-time-safe frozen columns), and it never deletes or backdates anything.
...
- `docs/epics/EPIC-040-continuous-self-learning-loop.md` — this completion report.
...
- [x] Active model changes are traceable to comparison evidence (`model_promotion_decision_id` foreign key into EPIC-039's immutable log).
...
I believe this implementation satisfies all eight acceptance criteria with real, verified evidence, including a real-Postgres migration round-trip and direct proof of the discovery-trigger hook firing exactly when it should. This EPIC composes EPIC-034/EPIC-038/EPIC-039's entry points wholesale and reuses EPIC-027's watermark trigger unchanged, adding only the wiring, lineage, and an optional, decoupled discovery-trigger hook as genuinely new logic. Per the user's standing-contract update, Claude will merge this PR once CI is green and it is cleanly mergeable, then continue to the next eligible EPIC.
