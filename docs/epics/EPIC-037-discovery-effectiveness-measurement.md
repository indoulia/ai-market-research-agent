# EPIC-037 — Discovery Effectiveness Measurement
...
## Dependencies
**Previous:** EPIC-023, EPIC-033, EPIC-034
**Next:** EPIC-038
...
autonomous/epic-m1-42, branched cleanly from `main` (all three declared dependencies -- EPIC-023, EPIC-033, EPIC-034 -- are already merged).

### Objective

Measure whether each discovery source and segment actually produces useful positive recommendations and successful outcomes, comparing sources fairly over the same time period and adding return magnitude and structural redundancy measures that EPIC-023's funnel did not cover.

### Funnel Definitions

`discovered_count` (a `DiscoveryRecord` exists) → `routed_count` (a `RecommendationGeneration` was produced) → `rejected_count` (`NOT_QUALIFIED`, a **candidate rejection**, never conflated with a later recommendation failure) vs. `qualified_count` (`QUALIFIED`) → among qualified, `evaluated_count`/`success_count`/`failure_count` once EPIC-033 has actually classified the outcome. Qualification rate is `qualified_count / routed_count`; recommendation rate is `qualified_count / discovered_count` -- both directly derivable from the funnel, never a separate metric requiring new state.

### Sample-Size Rules

Every success-rate verdict (`OK`/`WEAK`/`INSUFFICIENT_SAMPLE`, EPIC-023's own vocabulary, reused unchanged) and every redundancy verdict (`REDUNDANT`/`NOT_REDUNDANT`/`INSUFFICIENT_SAMPLE`, this EPIC's own) requires `MIN_SAMPLE_SIZE_FOR_COMPARISON` (EPIC-019, 20) evaluated/discovered rows before anything other than `INSUFFICIENT_SAMPLE` is possible (AC: "small samples are explicitly marked insufficient"). `rank_discovery_sources` excludes `INSUFFICIENT_SAMPLE` sources from the ranking entirely rather than placing them arbitrarily.

### Comparative Metrics

- **Common-period comparison** (scope: "compare discovery channels over common periods"): every query filters on `DiscoveryRecord.discovered_at` -- the one timestamp every candidate has regardless of whether it was ever routed or qualified -- against a caller-supplied `EvaluationWindow` (EPIC-074), so two sources with different lifetimes are never compared unfairly across different periods.
- **Return by source and segment** (scope, genuinely new vs. EPIC-023): `average_realized_return` per source and per (source, sector)/(source, market-cap-bucket)/(source, industry)/(source, regime), from EPIC-033's `OutcomeMeasurement.realized_return`.
- **Segmentation** (AC: "segmented by market, sector, size, and industry where applicable"): sector/market-cap-bucket/industry via EPIC-029's already-persisted `DiscoverySegment` (joined by its own `discovery_record_id` unique key, not reclassified), "market" interpreted as EPIC-021's market regime via `MarketRegime`, all reported "where available" per this platform's established honest-partial-coverage pattern -- a discovery without a `DiscoverySegment`/`MarketRegime` row simply doesn't contribute to that breakdown.
...
- **Reuses rather than duplicates**: EPIC-023's `VERDICT_OK`/`VERDICT_WEAK`/`VERDICT_INSUFFICIENT_SAMPLE` vocabulary (imported, not redefined), EPIC-019's `MIN_SAMPLE_SIZE_FOR_COMPARISON`/`WEAKNESS_MARGIN`, EPIC-074's `EvaluationWindow`, EPIC-029's persisted `DiscoverySegment`, and EPIC-033's `OutcomeMeasurement`. No existing module is modified.
- **Does not call EPIC-023's `compute_discovery_effectiveness_report` directly**: that function is unwindowed (lifetime-only), and this EPIC's core new requirement is a *common-period* comparison. Reimplementing a windowed variant of the same funnel tally (rather than adding a window parameter to EPIC-023's already-merged, already-tested function) keeps this EPIC additive and leaves EPIC-023 untouched.
...
- `docs/epics/EPIC-037-discovery-effectiveness-measurement.md` — this completion report.

No migration: pure read-side aggregation over existing tables, matching EPIC-023's own precedent.
...
I believe this implementation satisfies all six acceptance criteria with real, verified evidence, including concrete proof of weak-vs-strong source identification, redundancy detection, and common-period window exclusion. This EPIC composes EPIC-023's funnel vocabulary, EPIC-019's evidence-gating constants, EPIC-074's window abstraction, EPIC-029's persisted segmentation, and EPIC-033's outcome measurement rather than duplicating any of them, while adding genuinely new return-magnitude and redundancy measures EPIC-023 did not cover. Per the user's standing-contract update, Claude will merge this PR once CI is green and it is cleanly mergeable, then continue to the next eligible EPIC.
