...
- **`get_watchlist_decision_history(session, *, stock_id=None, symbol=None, start=None, end=None)`** orders deterministically by `decided_at` then `id`, satisfying scope item 6 ("deterministic history queries by symbol and time range"). `decided_at` is the watchlist analysis's own `discovered_at` timestamp (EPIC-020's provenance timestamp), not a separately re-derived value.
- Deliberately does not modify `app/watchlist_analysis.py` (EPIC-068), `app/discovery.py` (EPIC-020), or `app/recommendation_generator.py` (EPIC-016) at all -- `record_watchlist_decision` is a composable step a caller invokes after `analyze_watchlist_stock`, exactly like EPIC-029's `record_segments_for_scan` composes after EPIC-028's discovery step without touching it.
...
- `docs/epics/EPIC-069-watchlist-decision-history.md` — this completion report.
