# EPIC-080 — Prediction Trust Score Engine
...
## Dependency Chain
**Previous:** EPIC-072, EPIC-074, EPIC-045, EPIC-049, EPIC-062.
**Next:** EPIC-081, EPIC-084, EPIC-085, EPIC-087.
...
autonomous/epic-m1-77, branched cleanly from `main` (the declared dependencies -- EPIC-072, EPIC-074, EPIC-045, EPIC-049, EPIC-062 -- are already merged).
...
Scope names "recent performance," "model stability," and "drift signals" as distinct dimensions, but this platform currently has only one real, independently-computed signal for all three: EPIC-062's `ModelRegressionCheck` (a model's own real-world performance holding steady vs. regressing over time *is* this platform's current notion of stability/drift). Rather than inventing two more numbers from the same source to look more thorough, `recent_performance_component` alone represents all three -- an honest, forward-compatible choice matching EPIC-030's own prior posture toward data types with no independent signal yet.
...
If the prediction's own latest EPIC-078 `EvidenceQualityDecision` is `STATE_LEAKAGE_DETECTED`, the entire trust score is forced to `QUALITY_INSUFFICIENT_DATA` regardless of how good every other component looks -- a real safety violation is never averaged away (`test_evidence_leakage_forces_insufficient_data`).
...
- `docs/epics/EPIC-080-prediction-trust-score-engine.md` — this completion report.
