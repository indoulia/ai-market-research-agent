# EPIC-086 — Prediction Quality & Benchmark Measurement
...
## Dependency Chain
**Previous:** EPIC-080, EPIC-081, EPIC-084.
**Next:** EPIC-087.
...
autonomous/epic-m1-82, branched cleanly from `main` (the declared dependencies -- EPIC-080, EPIC-081, EPIC-084 -- are already merged).
...
`trust_reduction_recommended` (true when the average excess return over the benchmark is negative) is exposed for a future consumer (EPIC-087, this EPIC's own listed "Next" dependency) -- this module has no write path to `Prediction`, `ScanCandidate`, or `PredictionTrustScore` itself (`test_never_writes_to_predictions`).
...
- `docs/epics/EPIC-086-prediction-quality-benchmark-measurement.md` — this completion report.
